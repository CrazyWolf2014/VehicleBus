...
public class DoubleMetaphone implements StringEncoder {
    ...
    static {
        ...
        ES_EP_EB_EL_EY_IB_IL_IN_IE_EI_ER = new String[]{"ES", "EP", "EB", "EL", "EY", "IB", "IL", MessageVo.DIRECTION_IN, "IE", "EI", "ER"};
        L_T_K_S_N_M_B_Z = new String[]{"L", NDEFRecord.TEXT_WELL_KNOWN_TYPE, "K", "S", "N", "M", "B", "Z"};
    }
    ...
    private int handleSC(String value, DoubleMetaphoneResult result, int index) {
        if (charAt(value, index + 2) == 'H') {
            if (contains(value, index + 3, 2, "OO", "ER", Constants.DEFAULT_LANGUAGE, "UY", "ED", "EM")) {
                if (contains(value, index + 3, 2, "ER", Constants.DEFAULT_LANGUAGE)) {
                    result.append("X", "SK");
                } else {
                    result.append("SK");
                }
    ...
    private int handleT(String value, DoubleMetaphoneResult result, int index) {
        if (contains(value, index, 4, "TION")) {
            ...
            result.append('T');
            return contains(value, index + 1, 1, NDEFRecord.TEXT_WELL_KNOWN_TYPE, "D") ? index + 2 : index + 1;
        }
    }
    ...
    private boolean conditionCH1(String value, int index) {
        if (!(contains(value, 0, 4, "VAN ", "VON ") || contains(value, 0, 3, "SCH"))) {
            if (!(contains(value, index - 2, 6, "ORCHES", "ARCHIT", "ORCHID") || contains(value, index + 2, 1, NDEFRecord.TEXT_WELL_KNOWN_TYPE, "S"))) {
                if (!((contains(value, index - 1, 1, "A", "O", NDEFRecord.URI_WELL_KNOWN_TYPE, "E") || index == 0) && (contains(value, index + 2, 1, L_R_N_M_B_H_F_V_W_SPACE) || index + 1 == value.length() - 1))) {
                    return false;
                }
            }
        }

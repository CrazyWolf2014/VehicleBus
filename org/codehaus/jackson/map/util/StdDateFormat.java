...
public class StdDateFormat extends DateFormat {
    ...
    protected boolean looksLikeISO8601(String dateStr) {
        if (dateStr.length() >= 5 && Character.isDigit(dateStr.charAt(0)) && Character.isDigit(dateStr.charAt(3)) && dateStr.charAt(4) == SignatureVisitor.SUPER) {
            return true;
        }
        return false;
    }
    ...
    private static final boolean hasTimeZone(String str) {
        ...
        if (len >= 6) {
            ...
            if (c == SignatureVisitor.EXTENDS || c == SignatureVisitor.SUPER) {
                return true;
            }
            c = str.charAt(len - 5);
            if (c == SignatureVisitor.EXTENDS || c == SignatureVisitor.SUPER) {

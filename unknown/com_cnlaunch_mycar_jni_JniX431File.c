...
{   
    ...
    { 
        ...
        (*env)->DeleteLocalRef(env, strencode);//add by bcf
        (*env)->DeleteLocalRef(env, clsstring);//add by bcf
        (*env)->ReleaseByteArrayElements(env, barr, (jbyte*)str, 0); //add by bcf
        (*env)->DeleteLocalRef(env, barr);//add by bcf
    ...
    {
        char* rtn = NULL; 
        (*env)->DeleteLocalRef(env, rtn);  // add by bcf
        ...
        (*env)->DeleteLocalRef (env, jstr); // add by bc
        (*env)->DeleteLocalRef (env, strencode);  // add by bcf
...
#endif
#endif
        (*env)->DeleteLocalRef(env, obj);//add by bcf
    ...
    */
        ...
        (JNIEnv *env, jobject obj, jint hlsx, jobject filename, jint mode, jobject error)
    {
        ...
        iError = (int)(*env)->GetIntField(env, error, fid_mValue);
    ...
    */
        ...
        (JNIEnv *env, jobject obj, jint grp, jobjectArray textstrs, jint n)
    {
        ...
        for ( ; iTmp<iCount; iTmp++)
        {
            ...
            (*env)-> DeleteLocalRef(env, jstr);//add by bcf
        }
        (*env)-> DeleteLocalRef(env, textstrs);//add by bcf
    ...
    */
        ...
        (JNIEnv *env, jobject obj, jint grp, jobjectArray namestrs, jobjectArray unitstrs, jintArray type, jint n)
    {
        ...
        (*env)-> DeleteLocalRef(env, type); //add by bcf 1111
        ...
        for ( ; i < iCount; i++)
        {
            ...
            obj_name = (*env)->GetObjectArrayElement(env, namestrs, i);
            (*env)-> DeleteLocalRef(env, namestrs); //add by bcf 1111
            jstr_str_name = (*env)->GetObjectField(env, obj_name, fid_str);
            p_str_name = jstringToChar(env, jstr_str_name);
            (*env)-> DeleteLocalRef(env, jstr_str_name);//add by bcf 111
            pNames[i]->str = p_str_name;

            jstr_str_en_name = (*env)->GetObjectField(env, obj_name, fid_str_en);
            (*env)-> DeleteLocalRef(env, obj_name); //add by bcf
            p_str_en_name = jstringToChar(env, jstr_str_en_name);
            pNames[i]->str_en = p_str_en_name;
            (*env)-> DeleteLocalRef(env, jstr_str_en_name);//add by bcf 111

            obj_text = (*env)->GetObjectArrayElement(env, unitstrs, i);
            (*env)-> DeleteLocalRef(env, unitstrs); //add by bcf
            jstr_str_text = (*env)->GetObjectField(env, obj_text, fid_str);
            (*env)-> DeleteLocalRef(env, fid_str); //add by bcf 111

            p_str_text = jstringToChar(env, jstr_str_text);
            pUnits[i]->str = p_str_text;
            (*env)-> DeleteLocalRef(env, jstr_str_text);//add by bcf 111

            jstr_str_en = (*env)->GetObjectField(env, obj_text, fid_str_en);
            (*env)-> DeleteLocalRef(env, obj_text); //add by bcf
            (*env)-> DeleteLocalRef(env, fid_str_en); //add by bcf 111
            p_str_en_text = jstringToChar(env, jstr_str_en);

           pUnits[i]->str_en =p_str_en_text;
           (*env)-> DeleteLocalRef(env, jstr_str_en);//add by bcf 111
        }

        (*env)-> DeleteLocalRef(env, cls); //add by bcf 1111
        ...
        (*env)-> DeleteLocalRef(env, type); //add by bcf
    ...
    */
        ...
        (JNIEnv *env, jobject obj, jint grp, jobjectArray itemstrs, jint n)
    {
        ...
        if ( n <= 0 )
        {
            ...
            printf( "lsx_rec_writeds: parameter[n] invalid\n");
#else
            __android_log_print(ANDROID_LOG_ERROR, "JniX431FileTest", "lsx_rec_writeds: parameter[n] invalid");
#endif
#endif
            return (jint)-1;
        }        
        ...
        for ( ; i < iCount; i++)
        {
            ...
            (*env)-> DeleteLocalRef(env, itemstrs);//add by bcf
            jstr_str_item = (*env)->GetObjectField(env, obj_item, fid_str);
            //(*env)-> DeleteLocalRef(env, fid_str);//add by bcf
            p_str_item = jstringToChar(env, jstr_str_item);

            pItems[i]->str = p_str_item;
            //(*env)-> DeleteLocalRef(env, jstr_str_item);//add by bcf

            jstr_str_en_item = (*env)->GetObjectField(env, obj_item, fid_str_en);
            (*env)-> DeleteLocalRef(env, obj_item);//add by bcf
            //(*env)-> DeleteLocalRef(env, fid_str_en);//add by bcf
            p_str_en_item = jstringToChar(env, jstr_str_en_item);
            pItems[i]->str_en = p_str_en_item;
            //(*env)-> DeleteLocalRef(env, jstr_str_en_item);//add by bcf

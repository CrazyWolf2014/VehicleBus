...
public class CToJava {
    ...
    public static boolean isChexing;
    public static boolean isHistory;
    public static MemoryThread memoryThread;
    public static MemoryThreadPage memoryThreadPage;
    public static MemoryThreadVW memoryThreadVW;
    ...
    public static byte[] WriteAndRead2701(byte[] sendBuffer, int sendDataLength, byte[] receiveBuffer, int receiveBufferLength, int maxWaitTime) {
        ...
        if (MySharedPreferences.getStringValue(MainActivity.contexts, MySharedPreferences.generateOperatingRecord).equalsIgnoreCase(Contact.RELATION_ASK)) {
            ...
            Date date = new Date();
            String date2 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(r24);
    ...
    public static byte[] ReceiveDataByTime(byte[] receiveBuffer, int receiveBufferLength, int maxWaitTime) {
        StdJni stdJni = new StdJni();
        byte[] pReceiveBuffer = receiveBuffer;
        int iReceiveBufferLength = receiveBufferLength;
        int iMaxWaitTime = maxWaitTime;
        ...
        mChatService.setWaitTime(iMaxWaitTime);
        Runnable dynamic = new ReadByteData(new Bridge());
        BluetoothChatService.readData = XmlPullParser.NO_NAMESPACE;
    ...
    public static byte[] getProtData(byte[] databuffer) {
        ...
        PageInteractiveData pageInteractiveData = PageInteractiveDataAnalysis.dataUtil(feedbackBuffer);
        String path;
        int i;
        MenuData menuData;
        String lString;
        ...
        switch (pageInteractiveData.getPackageType()) {
            case MessageOptions.MESSAGE_SET_WIRE_FORMAT_FIELD_NUMBER /*1*/:
                ...
                if (path != null && path.length() > 0) {
                    ...
                    feedback = PageInteractiveDataAnalysis.feedbackData(backData, pageInteractiveData);
                    break;
                }
            ...
            case FileOptions.JAVA_MULTIPLE_FILES_FIELD_NUMBER /*10*/:
                ...
                feedback = PageInteractiveDataAnalysis.feedbackData(new byte[0], pageInteractiveData);
                searchId.ggpClose();
                break;
            ...
            case TrafficIncident.STARTTIME_FIELD_NUMBER /*14*/:
                ...
                if (path != null && path.length() > 0) {
                    for (i = 0; i < troubleTestList.size(); i++) {
                        ...
                        i2 = (databuf[2] & KEYRecord.PROTOCOL_ANY) * KEYRecord.OWNER_ZONE;
                        int v_id = ((((databuf[0] & KEYRecord.PROTOCOL_ANY) * 16777216) + ((databuf[1] & KEYRecord.PROTOCOL_ANY) * AccessibilityEventCompat.TYPE_VIEW_ACCESSIBILITY_FOCUS_CLEARED)) + r0) + (databuf[3] & KEYRecord.PROTOCOL_ANY);
                        troubleIdContentbytes = searchId.getTextFromLibReturnByte(v_id, 6);
                        troubleTest.setTroubleHelp(ByteHexHelper.byteToWord(searchId.getTextFromLibReturnByte(v_id, 8)));
                        ...
                        if (troubleIdContent == null || troubleIdContent.length <= 0) {
                            ...
                            troubleTest.setTroubleDescribeContent(new StringBuilder(String.valueOf(ByteHexHelper.replaceBlank1(troubleTest.getTroubleId()))).append(MinimalPrettyPrinter.DEFAULT_ROOT_VALUE_SEPARATOR).append(MainActivity.contexts.getResources().getString(C0136R.string.ReferManual)).toString());
                        } else {
            ...
            case FileOptions.JAVA_GENERIC_SERVICES_FIELD_NUMBER /*17*/:
                ...
                if (path != null && path.length() > 0) {
                    for (i = 1; i < streamSelectIdlist.size(); i++) {
                        ...
                        streamSelectIdItem.setSite(i);
                    }
                    if (streamSelectIdlist != null && streamSelectIdlist.size() > 1) {
                        streamSelectIdlist.remove(0);
                    }
                    Collections.sort(streamSelectIdlist);
            ...
            case Protocol.LEAF_2 /*26*/:
                if (!XmlPullParser.NO_NAMESPACE.equals(Constant.OBD2_EN_GGP_PATH_OLD)) {
                    searchId.ggpClose();
                }
            ...
            case Service.MSG_ICP /*29*/:
                ...
                Constant.feedback = null;
                feedback = PageInteractiveDataAnalysis.feedbackData(backData1, pageInteractiveData);
                break;
            case Protocol.NETBLT /*30*/:
                ...
                agingFlag = Boolean.valueOf(false);
                haveData = Boolean.valueOf(false);
                feedback = PageInteractiveDataAnalysis.feedbackData(Constant.continueObtain, pageInteractiveData);
                break;
            case Type.ATMA /*34*/:
                ...
                Constant.spt_data_stream = ByteHexHelper.hexStringToBytes(new StringBuilder(String.valueOf(ByteHexHelper.bytesToHexString(new byte[]{dataStreamEmptyBack[0], dataStreamEmptyBack[1], dataStreamCountBack[0], dataStreamCountBack[1], dataStreamButtonBack[0], dataStreamMaskLengthBack[1]}))).append(ByteHexHelper.bytesToHexString(((SptExDataStreamIdItem34) backList.get(0)).getChoseMaskBufferList())).toString());
            ...
            case Type.NAPTR /*35*/:
                Constant.isDataStreamChange = false;
                feedback = dataStreamItem35(pageInteractiveData, null);
                break;
            case Type.KX /*36*/:
                feedback = specialFunchtion(pageInteractiveData, null);
                break;
            case Service.TIME /*37*/:
                feedback = spt_trouble_bode_id_ex(pageInteractiveData, null, "SPT_TROUBLE_CODE_ID_EX", 37);
                break;
            case Type.A6 /*38*/:
                feedback = spt_trouble_bode_id_ex(pageInteractiveData, null, "SPT_TROUBLE_CODE_FROZEN_EX", 38);
                break;
            case Service.RLP /*39*/:
                feedback = spt_trouble_bode_id_ex(pageInteractiveData, null, "SPT_TROUBLE_CODE_ID_BY_COMBINE", 39);
                break;
            case SmileConstants.TOKEN_MISC_FP /*40*/:
                feedback = spt_trouble_bode_id_ex(pageInteractiveData, null, "SPT_TROUBLE_CODE_FROZEN_BY_COMBINE", 40);
                break;
            case Service.GRAPHICS /*41*/:
                feedback = spt_trouble_bode_id_ex(pageInteractiveData, null, "SPT_TROUBLE_CODE_ID_EX_BY_COMBINE", 41);
                break;
            case Service.NAMESERVER /*42*/:
                feedback = spt_trouble_bode_id_ex(pageInteractiveData, null, "SPT_TROUBLE_CODE_FROZEN_EX_BY_COMBINE", 42);
                break;
            case Service.NICNAME /*43*/:
                feedback = spt_trouble_bode_id_ex(pageInteractiveData, null, "SPT_TROUBLE_CODE_ID_BY_LOOPMODE", 43);
                break;
            case Service.MPM_FLAGS /*44*/:
                ...
                nowaitmessageboxtext = Boolean.valueOf(false);
                haveData = Boolean.valueOf(false);
                feedback = PageInteractiveDataAnalysis.feedbackData(Constant.noWaitMessageButton, pageInteractiveData);
                break;
            case Service.MPM /*45*/:
                ...
                progressbarFlag = Boolean.valueOf(false);
                haveData = Boolean.valueOf(false);
                feedback = PageInteractiveDataAnalysis.feedbackData(Constant.progressbarBox, pageInteractiveData);
                break;
            case Service.MPM_SND /*46*/:
                ...
                combinationFlag = Boolean.valueOf(false);
                haveData = Boolean.valueOf(false);
                feedback = PageInteractiveDataAnalysis.feedbackData(Constant.CombinationMenu, pageInteractiveData);
                break;
            ...
            case Type.DNSKEY /*48*/:
                ...
                if (!(vincode == null || vincode.equalsIgnoreCase(XmlPullParser.NO_NAMESPACE))) {
                    ...
                    byte[] mVinBack1 = new byte[(hexStrLen + 1)];
                    mVinBack1[0] = ByteHexHelper.intToHexByte(hexStrLen);
                    for (i = 1; i < hexStrLen + 1; i++) {
                        mVinBack1[i] = mVinBack[i - 1];
                    }
                    feedback = PageInteractiveDataAnalysis.feedbackData(mVinBack1, pageInteractiveData);
                    break;
                }
            case Service.LOGIN /*49*/:
                ...
                if (path2 != null && path2.length() > 0) {
                    intent = new Intent("feedbackMeauWithHelpData");
                    Bundle bundle49 = new Bundle();
                    bundle49.putSerializable("menuData", menuDataList2);
                    bundle49.putInt(SharedPref.TYPE, 1);
                    ...
                    Constant.feedback = null;
                    feedback = PageInteractiveDataAnalysis.feedbackData(backData49, pageInteractiveData);
                    break;
                }
            case Type.NSEC3 /*50*/:
                ...
                bundle50.putSerializable("menuData", menuDataList3);
                bundle50.putInt(SharedPref.TYPE, 1);
                ...
                Constant.feedback = null;
                feedback = PageInteractiveDataAnalysis.feedbackData(backData50, pageInteractiveData);
                break;
            case Service.LA_MAINT /*51*/:
                ...
                break;
            case Type.TLSA /*52*/:
                length = PageInteractiveDataAnalysis.SPT_GGP_PATHNAME(pageInteractiveData).length;
                if (r0 > 1) {
                    feedback = PageInteractiveDataAnalysis.feedbackData(Constant.streamNextCode, pageInteractiveData);
                    break;
                }
            ...
            case SimpleResolver.DEFAULT_PORT /*53*/:
                ...
                intent = new Intent("showdiagpicture");
                ...
                Constant.feedback = null;
                feedback = PageInteractiveDataAnalysis.feedbackData(backData53, pageInteractiveData);
                break;
            case Opcodes.ISTORE /*54*/:
                feedback = PageInteractiveDataAnalysis.feedbackData(MySharedPreferences.getStringValue(MainActivity.contexts, MySharedPreferences.versionName).getBytes(), pageInteractiveData);
                break;
            case SmileConstants.MAX_SHORT_NAME_UNICODE_BYTES /*56*/:
                byte[] feedbackBuffer2 = new byte[(feedbackBuffer.length - 2)];
                if (new StringBuilder(String.valueOf(feedbackBuffer[7] + feedbackBuffer[8])).toString().equalsIgnoreCase(Contact.RELATION_BACKNAME)) {
                    isChexing = true;
                } else {
                    isChexing = false;
                }
                ArrayList<MenuData> cxmenuDataList = PageInteractiveDataAnalysis.spt_menu_id56(pageInteractiveData);
                String pathcx = Constant.OBD2_EN_GGP_PATH;
                if (pathcx != null && pathcx.length() > 0) {
                    for (i = 1; i < cxmenuDataList.size(); i++) {
                        menuData = (MenuData) cxmenuDataList.get(i);
                        lString = XmlPullParser.NO_NAMESPACE;
                        databuf = ByteHexHelper.hexStringToBytes(menuData.getMenuId());
                        i2 = (databuf[2] & KEYRecord.PROTOCOL_ANY) * KEYRecord.OWNER_ZONE;
                        MenuContentbytes = searchId.getTextFromLibReturnByte(((((databuf[0] & KEYRecord.PROTOCOL_ANY) * 16777216) + ((databuf[1] & KEYRecord.PROTOCOL_ANY) * AccessibilityEventCompat.TYPE_VIEW_ACCESSIBILITY_FOCUS_CLEARED)) + r0) + (databuf[3] & KEYRecord.PROTOCOL_ANY), 1);
                    ...
                    bundle56.putSerializable("menuData", cxmenuDataList);
                    bundle56.putInt(SharedPref.TYPE, 1);
                    intent.putExtras(bundle56);
                    MainActivity.contexts.sendBroadcast(intent);
                    thread = new Thread(new CarDiagnose(new CarDiagnoseBridge()));
                    thread.start();
                    try {
                    ...
                    byte[] backData56 = Constant.feedback;
                    Constant.feedback = null;
                    feedback = PageInteractiveDataAnalysis.feedbackData(backData56, pageInteractiveData);
                    break;
                }
            case Service.TACACS_DS /*65*/:
                feedback = PageInteractiveDataAnalysis.feedbackData(new byte[]{(byte) 0}, pageInteractiveData);
                break;
            case Service.CSNET_NS /*105*/:
                int packageType = ByteHexHelper.byteToInt(pageInteractiveData.getData()[0]);
                feedback = PageInteractiveDataAnalysis.feedbackData(new byte[]{(byte) 0}, pageInteractiveData);
                break;
        }
        return feedback;
    ...
    private static byte[] dataStreamItem34(PageInteractiveData uiData, byte[] feedback) {
        ...
        if (!(MySharedPreferences.getStringValue(MainActivity.contexts, MySharedPreferences.conversion) == null || MySharedPreferences.getStringValue(MainActivity.contexts, MySharedPreferences.conversion).equals(XmlPullParser.NO_NAMESPACE) || !MySharedPreferences.getStringValue(MainActivity.contexts, MySharedPreferences.conversion).equals("Imperial"))) {
    ...
    private static byte[] spt_trouble_bode_id_ex(PageInteractiveData uiData, byte[] feedback, String action, int type) {
        Log.i("CTO ", "\u663e\u793a\u6545\u969c\u7801spt_trouble_bode_id_ex");
        ...
        Log.i("CTO ", new StringBuilder(SharedPref.TYPE).append(type).toString());
        intent.putExtras(bundle);
        MainActivity.contexts.sendBroadcast(intent);
    ...
    public static void remoteSendClickData(int type, ArrayList<?> arrayList, Object object) {
        JSONObject jSONObject;
        try {
            JSONObject json = new JSONObject();
            try {
                json.put(SharedPref.TYPE, type);
                json.put("app", (Object) "android");
                json.put("mode", (Object) "CMD");
                json.put(DataPacketExtension.ELEMENT_NAME, object.toString());
                sendDataString(json.toString());
                jSONObject = json;
            } catch (Exception e) {
                jSONObject = json;
            }
    ...
    public static void sendDataString(String dataStr) {
        MainActivity.socketSendThread.addListInArrary(dataStr);
        if (MainActivity.socketSendThread.isWait()) {
            MainActivity.socketSendThread.notifyThread();
        }
    }

    public static void resendDataString() {
        if (MainActivity.socketSendThread != null && MainActivity.socketSendThread.isWait()) {
            MainActivity.socketSendThread.notifyThread();
        }
    }
}

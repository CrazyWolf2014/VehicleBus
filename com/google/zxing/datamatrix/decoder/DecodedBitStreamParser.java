...
final class DecodedBitStreamParser {
    ...
    static {
        ...
        C40_SHIFT2_SET_CHARS = new char[]{'!', '\"', '#', '$', '%', '&', '\'', '(', ')', '*', SignatureVisitor.EXTENDS, ',', SignatureVisitor.SUPER, '.', '/', ':', ';', '<', SignatureVisitor.INSTANCEOF, '>', '?', '@', '[', '\\', ']', '^', '_'};
    ...
    private static int decodeAsciiSegment(BitSource bitSource, StringBuffer stringBuffer, StringBuffer stringBuffer2) throws FormatException {
        ...
        do {
            ...
            if (readBits == 0) {
                throw FormatException.getFormatInstance();
            } else if (readBits <= Flags.FLAG8) {
                stringBuffer.append((char) ((i != 0 ? readBits + Flags.FLAG8 : readBits) - 1));
                return ASCII_ENCODE;
            } else if (readBits == Service.PWDGEN) {
                return PAD_ENCODE;
            } else {
                if (readBits <= 229) {
                    readBits -= 130;
    ...
    private static void decodeBase256Segment(BitSource bitSource, StringBuffer stringBuffer, Vector vector) throws FormatException {
        ...
        } else if (readBits >= Type.TSIG) {
            readBits = ((readBits - 249) * Type.TSIG) + bitSource.readBits(8);
        }
                    ...
                    switch (readBits) {
                        case PAD_ENCODE /*0*/:
                            if (i4 >= TEXT_ENCODE) {
                                ...
                                stringBuffer.append((char) (C40_BASIC_SET_CHARS[i4] + Flags.FLAG8));
                                i3 = readBits;
                                obj = PAD_ENCODE;
                                break;
                            }
                        ...
                        case ASCII_ENCODE /*1*/:
                            if (v != null) {
                                stringBuffer.append((char) (i4 + Flags.FLAG8));
                                v = PAD_ENCODE;
                            } else {
                        ...
                        case C40_ENCODE /*2*/:
                            if (i4 < 27) {
                                if (v != null) {
                                    stringBuffer.append((char) (C40_SHIFT2_SET_CHARS[i4] + Flags.FLAG8));
                                    v = PAD_ENCODE;
                                } else {
                    ...
                    switch (readBits) {
                        case PAD_ENCODE /*0*/:
                            if (i4 >= TEXT_ENCODE) {
                                ...
                                stringBuffer.append((char) (TEXT_BASIC_SET_CHARS[i4] + Flags.FLAG8));
                                i3 = readBits;
                                obj = PAD_ENCODE;
                                break;
                            }
                        ...
                        case TEXT_ENCODE /*3*/:
                            if (v != null) {
                                stringBuffer.append((char) (TEXT_SHIFT3_SET_CHARS[i4] + Flags.FLAG8));
                                v = PAD_ENCODE;
                            } else {
    ...
    private static byte unrandomize255State(int i, int i2) {
        int i3 = i - (((i2 * Opcodes.FCMPL) % KEYRecord.PROTOCOL_ANY) + ASCII_ENCODE);
        if (i3 < 0) {

...
public class GlobalHistogramBinarizer extends Binarizer {
    ...
    public BitMatrix getBlackMatrix() throws NotFoundException {
        ...
        for (i = 1; i < LUMINANCE_BITS; i++) {
            ...
            for (i2 = width / LUMINANCE_BITS; i2 < i3; i2++) {
                int i4 = (row[i2] & KEYRecord.PROTOCOL_ANY) >> LUMINANCE_SHIFT;
                iArr[i4] = iArr[i4] + 1;
            }
        }
        ...
        for (i = 0; i < height; i++) {
            ...
            for (i2 = 0; i2 < width; i2++) {
                if ((matrix[i5 + i2] & KEYRecord.PROTOCOL_ANY) < estimateBlackPoint) {
                    bitMatrix.set(i2, i);
                }
            }
        }
    ...
    public BitArray getBlackRow(int i, BitArray bitArray) throws NotFoundException {
        ...
        int[] iArr = this.buckets;
        for (i2 = 0; i2 < width; i2++) {
            i3 = (row[i2] & KEYRecord.PROTOCOL_ANY) >> LUMINANCE_SHIFT;
            iArr[i3] = iArr[i3] + 1;
        }
        i3 = estimateBlackPoint(iArr);
        i2 = row[1] & KEYRecord.PROTOCOL_ANY;
        int i5 = row[0] & KEYRecord.PROTOCOL_ANY;
        while (i4 < width - 1) {
            int i6 = row[i4 + 1] & KEYRecord.PROTOCOL_ANY;
            if (((((i2 << 2) - i5) - i6) >> 1) < i3) {
                bitArray.set(i4);
            }
            i4++;

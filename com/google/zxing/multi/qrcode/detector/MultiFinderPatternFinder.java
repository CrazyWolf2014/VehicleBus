...
final class MultiFinderPatternFinder extends FinderPatternFinder {
    ...
    private static class ModuleSizeComparator implements Comparator {
        ...
        ModuleSizeComparator(C02421 c02421) {
            this();
        }
                        ...
                        if (finderPattern2 != null) {
                            ...
                            for (int i3 = i2 + 1; i3 < size; i3++) {
                                ...
                                if (finderPattern3 != null) {
                                    ...
                                    if (estimatedModuleSize3 <= MAX_MODULE_COUNT_PER_EDGE && estimatedModuleSize3 >= MIN_MODULE_COUNT_PER_EDGE && Math.abs((distance - estimatedModuleSize) / Math.min(distance, estimatedModuleSize)) < Binarize.OTSU_SCORE_FRACTION) {
                                        estimatedModuleSize = (float) Math.sqrt((double) ((estimatedModuleSize * estimatedModuleSize) + (distance * distance)));
                                        if (Math.abs((distance2 - estimatedModuleSize) / Math.min(distance2, estimatedModuleSize)) < Binarize.OTSU_SCORE_FRACTION) {

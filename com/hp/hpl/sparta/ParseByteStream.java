...
class ParseByteStream implements ParseSource {
    ...
    private static boolean equals(byte[] bArr, int i) {
        return bArr[0] == ((byte) (i >>> 24)) && bArr[1] == ((byte) ((i >>> 16) & KEYRecord.PROTOCOL_ANY)) && bArr[2] == ((byte) ((i >>> 8) & KEYRecord.PROTOCOL_ANY)) && bArr[3] == ((byte) (i & KEYRecord.PROTOCOL_ANY));
    }

    private static boolean equals(byte[] bArr, short s) {
        return bArr[0] == ((byte) (s >>> 8)) && bArr[1] == ((byte) (s & KEYRecord.PROTOCOL_ANY));
    }

    private static String fixEncoding(String str) {
        return str.toLowerCase().equals("utf8") ? AsyncHttpResponseHandler.DEFAULT_CHARSET : str;
    }
    ...
    private static String guessEncoding(String str, byte[] bArr, int i, ParseLog parseLog) throws IOException {
        ...
        if (i != 4) {
            ...
            str2 = (equals(bArr, 65279) || equals(bArr, -131072) || equals(bArr, 65534) || equals(bArr, -16842752) || equals(bArr, 60) || equals(bArr, 1006632960) || equals(bArr, 15360) || equals(bArr, 3932160)) ? "UCS-4" : equals(bArr, 3932223) ? "UTF-16BE" : equals(bArr, 1006649088) ? "UTF-16LE" : equals(bArr, 1010792557) ? AsyncHttpResponseHandler.DEFAULT_CHARSET : equals(bArr, 1282385812) ? "EBCDIC" : (equals(bArr, (short) -2) || equals(bArr, (short) -257)) ? "UTF-16" : AsyncHttpResponseHandler.DEFAULT_CHARSET;
        }
        if (!str2.equals(AsyncHttpResponseHandler.DEFAULT_CHARSET)) {
            parseLog.note(new StringBuffer().append("From start ").append(hex(bArr[0])).append(MinimalPrettyPrinter.DEFAULT_ROOT_VALUE_SEPARATOR).append(hex(bArr[1])).append(MinimalPrettyPrinter.DEFAULT_ROOT_VALUE_SEPARATOR).append(hex(bArr[2])).append(MinimalPrettyPrinter.DEFAULT_ROOT_VALUE_SEPARATOR).append(hex(bArr[3])).append(" deduced encoding = ").append(str2).toString(), str, 1);
        }
        return str2;
    }
    ...
    private static String hex(byte b) {
        ...
        switch (toHexString.length()) {
            case MessageOptions.MESSAGE_SET_WIRE_FORMAT_FIELD_NUMBER /*1*/:
                return new StringBuffer().append(Contact.RELATION_ASK).append(toHexString).toString();

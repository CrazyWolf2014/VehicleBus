...
public final class TrafficStats {
    ...
    public static void update() {
        ...
        try {
            ...
            if (br < 0) {
                br = j;
                Log.m1664v("MicroMsg.SDK.TrafficStats", "fix loss newMobileTx %d", Long.valueOf(j));
            }
            if (bs < 0) {
                bs = j2;
                Log.m1664v("MicroMsg.SDK.TrafficStats", "fix loss newMobileRx %d", Long.valueOf(j2));
            }
            if (bt < 0) {
                bt = j3;
                Log.m1664v("MicroMsg.SDK.TrafficStats", "fix loss newWifiTx %d", Long.valueOf(j3));
            }
            if (bu < 0) {
                bu = j4;
                Log.m1664v("MicroMsg.SDK.TrafficStats", "fix loss newWifiRx %d", Long.valueOf(j4));
            }
            if (j4 - bu < 0) {
                Log.m1664v("MicroMsg.SDK.TrafficStats", "minu %d", Long.valueOf(j4 - bu));
            }
            if (j3 - bt < 0) {
                Log.m1664v("MicroMsg.SDK.TrafficStats", "minu %d", Long.valueOf(j3 - bt));
            }
            bv = j >= br ? j - br : j;
            bw = j2 >= bs ? j2 - bs : j2;
            bx = j3 >= bt ? j3 - bt : j3;
        ...
        Log.m1656d("MicroMsg.SDK.TrafficStats", "current system traffic: wifi rx/tx=%d/%d, mobile rx/tx=%d/%d", Long.valueOf(by), Long.valueOf(bx), Long.valueOf(bw), Long.valueOf(bv));
    }

...
public class TimeLogger {
    ...
    public void dumpToLog() {
        if (!this.bo) {
            Log.m1655d(this.bm, this.bn + ": begin");
            long longValue = ((Long) this.bp.get(0)).longValue();
            int i = 1;
            long j = longValue;
            while (i < this.bp.size()) {
                long longValue2 = ((Long) this.bp.get(i)).longValue();
                Log.m1655d(this.bm, this.bn + ":      " + (longValue2 - ((Long) this.bp.get(i - 1)).longValue()) + " ms, " + ((String) this.bq.get(i)));
                i++;
                j = longValue2;
            }
            Log.m1655d(this.bm, this.bn + ": end, " + (j - longValue) + " ms");
        }
    }

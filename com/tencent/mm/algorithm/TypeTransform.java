...
public final class TypeTransform {
    ...
    private static byte[] m1641a(byte[] bArr) {
    ...
    public static int byteArrayHLToInt(byte[] bArr, int i) {
        return ((((bArr[i] & KEYRecord.PROTOCOL_ANY) << 24) | ((bArr[i + 1] & KEYRecord.PROTOCOL_ANY) << 16)) | ((bArr[i + 2] & KEYRecord.PROTOCOL_ANY) << 8)) | ((bArr[i + 3] & KEYRecord.PROTOCOL_ANY) << 0);
    }
    ...
    public static int byteArrayLHToInt(byte[] bArr, int i) {
        return ((((bArr[i + 3] & KEYRecord.PROTOCOL_ANY) << 24) | ((bArr[i + 2] & KEYRecord.PROTOCOL_ANY) << 16)) | ((bArr[i + 1] & KEYRecord.PROTOCOL_ANY) << 8)) | ((bArr[i] & KEYRecord.PROTOCOL_ANY) << 0);
    }
    ...
    public static byte[] intToByteArrayHL(int i) {
        return m1641a(intToByteArrayLH(i));
    }

    public static byte[] intToByteArrayLH(int i) {
        byte[] bArr = new byte[4];
        for (int i2 = 0; i2 < 4; i2++) {
            bArr[i2] = (byte) ((i >> (i2 * 8)) & KEYRecord.PROTOCOL_ANY);
        }
        return bArr;
    }

    public static byte[] longToByteArrayHL(long j) {
        return m1641a(longToByteArrayLH(j));
    }

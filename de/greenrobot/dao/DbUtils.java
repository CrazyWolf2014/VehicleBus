...
public class DbUtils {
    ...
    public static int executeSqlScript(Context context, SQLiteDatabase db, String assetFilename, boolean transactional) throws IOException {
        int count;
        String[] lines = new String(readAsset(context, assetFilename), AsyncHttpResponseHandler.DEFAULT_CHARSET).split(";(\\s)*[\n\r]");
        if (transactional) {
            count = executeSqlStatementsInTx(db, lines);
        } else {
            count = executeSqlStatements(db, lines);
        }
        DaoLog.m1705i("Executed " + count + " statements from SQL script '" + assetFilename + "'");
        return count;
    }
    ...
    public static void logTableDump(SQLiteDatabase db, String tablename) {
        ...
        try {
            DaoLog.m1701d(DatabaseUtils.dumpCursorToString(cursor));
        } finally {
            cursor.close();
        }
    }
